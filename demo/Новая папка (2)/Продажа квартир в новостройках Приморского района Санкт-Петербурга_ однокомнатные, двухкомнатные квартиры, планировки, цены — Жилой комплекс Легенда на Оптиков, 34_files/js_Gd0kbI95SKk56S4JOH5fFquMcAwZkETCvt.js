...
/*		$(".b-results-table tr ").live({
	   ...
	   $(".j-clear").click(function(){
		...
		if (br != -1 && br1 == -1)
		{
			...
			$(".j-camera-link").colorbox({inline:true, href:"#camera-popup", onOpen: function() {
				...
				flowplayer("player", Drupal.settings.optikov.flowplayer.swf,
					{
						clip: {
							url: 'soptik.stream',
							live: true,
						...
						plugins: {
							rtmp: {
								url: Drupal.settings.optikov.flowplayer.swfrtmp,
								netConnectionUrl: 'rtmp://wowza4.legenda-dom.ru/live'
							}
						}
					});
			}});
	  ...
	  $(".b-window-menu li").click(function(){
		...
		if ( $('.node-type-plans .l-large-gallery ul.small-gallery').find('li').length > 4 ) {
			galSlider = $('.node-type-plans .l-small-gallery ul.small-gallery').bxSlider({
				minSlides: 4,
				maxSlides: 4,
				slideWidth: 189,
				slideMargin: 3,
